...
void LocalMapping::WhetherOverlapObject()
{
    ...
    for(int i = 0; i < (int)vObjs.size(); ++i)
    {
        ...
        for(int j = 0; j < (int)vObjs.size(); ++j)
        {
            ...
            float dis_x = abs(Obj->cuboidCenter(0) - Obj2->cuboidCenter(0));
            float dis_y = abs(Obj->cuboidCenter(1) - Obj2->cuboidCenter(1));
            float dis_z = abs(Obj->cuboidCenter(2) - Obj2->cuboidCenter(2));

            float sum_lenth_half = Obj->lenth/2 + Obj2->lenth/2;
            float sum_width_half = Obj->width/2 + Obj2->width/2;
            float sum_height_half = Obj->height/2 + Obj2->height/2;

            float fVolume = (Obj->lenth * Obj->width) * Obj->height;
            float fVolume2 = (Obj2->lenth * Obj2->width) * Obj2->height;
